...
class RNN:
    ...
    def initmodel(self, x_train) -> None:
        '''
        ...
        model.add(layers.SimpleRNN(256, input_dim=x_train.shape[2], activation='relu'))
    ...
    def savemodel(self, file_name: str = 'RNN') -> None:
...
if __name__ == '__main__':
    ...
    axs[0].plot(rnn.trainHistory.epoch, rnn.trainHistory.history['loss'], label="loss")
    axs[0].set_ylabel('loss')
    axs[0].set_xlabel('epoch')
    axs[0].set_title('loss')
    axs[0].legend()

    axs[1].plot(rnn.trainHistory.epoch, rnn.trainHistory.history['accuracy'], label="accuracy")
    # axs[1].set_aspect(3)  # 压缩x轴与y轴的长度比，传入3表示x轴长度为y轴的3倍
    axs[1].set_ylabel('accuracy')
    axs[1].set_xlabel('epoch')
    axs[1].set_title('accuracy')
    axs[1].legend()
    fig.suptitle('RNN plots')
    fig.show()
